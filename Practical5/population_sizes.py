import sys #确保 Matplotlib 所在的路径在其中
sys.path.append('C:/Users/Hellen/AppData/Local/Microsoft/WindowsApps/python3.11.exe')
import matplotlib.pyplot as plt
uk_countries=[57.11,3.13,1.91,5.45]
uk_labels="England","Wales","Northern Ireland","Scotland"
plt.pie(uk_countries,explode=(0,0,0,0),labels=uk_labels,autopct="%1.1f%%",shadow=True,startangle=0)
plt.show()
zj_province=[65.77,41.88,45.28,61.27,85.15]
zj_labels="Zhejiang","Fujian","Jiangxi","Anhui","Jiangsu"
plt.pie(zj_province,explode=(0.2,0,0,0,0),labels=zj_labels,autopct="%1.1f%%",shadow=False,startangle=0)
plt.show()
uk_countries1=sorted(uk_countries)
zj_province1=sorted(zj_province)
print(uk_countries1,zj_province1)

